...
def _save_workbook_with_fallback(wb, output_path: str, fallback_suffix: str = "_results") -> str:
    try:
        wb.save(output_path)
        return output_path
    except PermissionError:
        output_file = Path(output_path)
        fallback_path = str(output_file.with_name(f"{output_file.stem}{fallback_suffix}{output_file.suffix}"))
        wb.save(fallback_path)
        print(f"Warning: Could not write to '{output_path}' because it is locked. Saved results to '{fallback_path}' instead.")
